...
export function RegisterForm() {
  ...
  const onSubmit = async (data: RegisterSchema) => {
    ...
    if (error) {
      setAuthError(error.message || 'Terjadi kesalahan saat mendaftar');
      return;
    }
    
    setAuthSuccess('Pendaftaran berhasil! Silakan cek email Anda untuk verifikasi.');
  ...
  return (
    <form className="form flex flex-col gap-3" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      {authSuccess && (
        <div className="text-positive text-sm text-center mb-2 bg-positive/10 p-2 rounded">
          {authSuccess}
        </div>
      )}
      
      <div className="form__group flex flex-col gap-1">
        <label className="form__label font-body text-[0.8rem] font-medium text-fg-2 tracking-[0.01em]" htmlFor="fullName">Nama Lengkap</label>
        ...
        <div className={`form__error text-[0.75rem] text-negative min-h-0 overflow-hidden transition-all ${errors.fullName ? 'min-h-[1.2rem] mt-1' : ''}`}>
      ...
      <div className="form__group flex flex-col gap-1">
        <label className="form__label font-body text-[0.8rem] font-medium text-fg-2 tracking-[0.01em]" htmlFor="confirmPassword">Konfirmasi Password</label>
        <div className="form__input-wrap flex items-center bg-bg border border-border rounded-lg overflow-hidden transition-all focus-within:border-accent focus-within:ring-[3px] focus-within:ring-accent/10">
          <input 
        ...
        <div className={`form__error text-[0.75rem] text-negative min-h-0 overflow-hidden transition-all ${errors.confirmPassword ? 'min-h-[1.2rem] mt-1' : ''}`}>

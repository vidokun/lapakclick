...
export function LoginForm() {
  ...
  const handleFormSubmit = async (e: React.FormEvent | React.KeyboardEvent) => {
  ...
  return (
    <div className="flex flex-col gap-3" onKeyDown={(e) => { if (e.key === 'Enter') handleFormSubmit(e); }}>
      {authError && (
      ...
      <div className="flex flex-col gap-1">
        <label className="font-body text-[0.8rem] font-medium text-fg-2 tracking-[0.01em]" htmlFor="email">Email atau Username</label>
        <div className="flex items-center bg-bg border border-border rounded-lg overflow-hidden transition-all focus-within:border-accent focus-within:ring-[3px] focus-within:ring-accent/10">
          <input 
            type="text" 
            className="flex-1 font-body text-[0.9rem] py-3 px-4 bg-transparent border-none text-fg outline-none min-w-0 placeholder:text-muted" 
            id="email" 
      ...
      <div className="flex flex-col gap-1">
        <label className="font-body text-[0.8rem] font-medium text-fg-2 tracking-[0.01em]" htmlFor="password">Password</label>
        <div className="flex items-center bg-bg border border-border rounded-lg overflow-hidden transition-all focus-within:border-accent focus-within:ring-[3px] focus-within:ring-accent/10">
          <input 
          ...
          <button 
            type="button" 
            className="flex items-center justify-center w-10 h-10 shrink-0 bg-transparent border-none text-muted text-base cursor-pointer transition-colors hover:text-fg" 
      ...
      <div className="flex items-center justify-between text-[0.8rem] mt-1">
        ...
        <Link href="/forgot-password" className="text-muted hover:text-accent transition-colors">Lupa password?</Link>
      </div>
      ...
      <div className="mt-2">
        <button 
          ...
          className="w-full inline-flex items-center justify-center gap-2 font-body font-medium text-[0.85rem] py-2 px-5 bg-accent text-bg border-none rounded hover:bg-accent-gl hover:-translate-y-[1px] cursor-pointer transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          disabled={isLoading}
        >
          {isLoading ? (
      ...
      <div className="flex items-center gap-4 my-2 before:flex-1 before:h-[1px] before:bg-border after:flex-1 after:h-[1px] after:bg-border">
      ...
      <div className="flex flex-col gap-2">
        <button 
          ...
          className="flex items-center justify-center gap-2 w-full py-[0.65rem] px-4 bg-surface-2 border border-border rounded-lg text-fg-2 font-body text-[0.85rem] font-medium cursor-pointer transition-all hover:border-accent-dim hover:text-fg hover:bg-surface"
        >
